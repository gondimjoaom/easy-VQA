...
NUM_TRAIN = 4000 if len(sys.argv) == 1 else int(sys.argv[1])
NUM_TEST = 1000 if len(sys.argv) == 1 else int(sys.argv[2])
location = None if len(sys.argv) == 1 else str(sys.argv[3])
seed(198)
...
def create_data(image_path, num):
  ...
  for i in range(num):
    ...
    create_image(f'{image_path}/{i}.png', shape, color, location=location)
...
with open('../easy_vqa/data/train/questions.json', 'w') as file:
  json.dump(train_questions, file, indent=4)
